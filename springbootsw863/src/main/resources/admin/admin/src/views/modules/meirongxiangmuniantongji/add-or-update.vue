<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.xiangmuzhaopian" label="项目照片" prop="xiangmuzhaopian">
          <file-upload
          tip="点击上传项目照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xiangmuzhaopian?ruleForm.xiangmuzhaopian:''"
          @change="xiangmuzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.xiangmuzhaopian" label="项目照片" prop="xiangmuzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xiangmuzhaopian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="美容部位" prop="meirongbuwei">
          <el-input v-model="ruleForm.meirongbuwei" 
              placeholder="美容部位" clearable  :readonly="ro.meirongbuwei"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="统计日期" prop="tongjiriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.tongjiriqi" 
                type="date"
                placeholder="统计日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tongjiriqi" label="统计日期" prop="tongjiriqi">
              <el-input v-model="ruleForm.tongjiriqi" 
                placeholder="统计日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="年销量" prop="nianxiaoliang">
          <el-input v-model="ruleForm.nianxiaoliang" 
              placeholder="年销量" clearable  :readonly="ro.nianxiaoliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="年销量" prop="nianxiaoliang">
              <el-input v-model="ruleForm.nianxiaoliang" 
                placeholder="年销量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="年销售额" prop="nianxiaoshoue">
            <el-input v-model="nianxiaoshoue"
                placeholder="年销售额" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.nianxiaoshoue" label="年销售额" prop="nianxiaoshoue">
              <el-input v-model="ruleForm.nianxiaoshoue" 
                placeholder="年销售额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
...
export default {
  ...
  computed: {
    nianxiaoshoue:{
      get: function () {
        return 1*this.ruleForm.xiangmujiage*this.ruleForm.nianxiaoliang
      }
    },
  },
  created() {
	this.ruleForm.tongjiriqi = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiangmumingcheng = true;
          ...
          if(o=='xiangmuzhaopian'){
            this.ruleForm.xiangmuzhaopian = obj[o];
	    this.ro.xiangmuzhaopian = true;
            continue;
          }
          if(o=='meirongbuwei'){
	    ...
	    this.ro.meirongbuwei = true;
          ...
          if(o=='tongjiriqi'){
            this.ruleForm.tongjiriqi = obj[o];
	    this.ro.tongjiriqi = true;
            continue;
          }
          if(o=='xiangmujiage'){
	    ...
	    this.ro.xiangmujiage = true;
          ...
          if(o=='nianxiaoliang'){
            this.ruleForm.nianxiaoliang = obj[o];
	    this.ro.nianxiaoliang = true;
            continue;
          }
          if(o=='nianxiaoshoue'){
            this.ruleForm.nianxiaoshoue = obj[o];
	    this.ro.nianxiaoshoue = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `meirongxiangmuniantongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      // ${column.compare}
      // ${column.compare}
        this.ruleForm.nianxiaoshoue = this.nianxiaoshoue
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `meirongxiangmuniantongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.meirongxiangmuniantongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    xiangmuzhaopianUploadChange(fileUrls) {
	this.ruleForm.xiangmuzhaopian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
