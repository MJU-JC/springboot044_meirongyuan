<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="技师工号" prop="jishigonghao">
          <el-select  @change="jishigonghaoChange" v-model="ruleForm.jishigonghao" placeholder="请选择技师工号">
            <el-option
                v-for="(item,index) in jishigonghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.jishigonghao" label="技师工号" prop="jishigonghao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="服务次数" prop="fuwucishu">
          <el-input v-model="ruleForm.fuwucishu" 
              placeholder="服务次数" clearable  :readonly="ro.fuwucishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="服务次数" prop="fuwucishu">
              <el-input v-model="ruleForm.fuwucishu" 
                placeholder="服务次数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingbie = true;
          ...
          if(o=='fuwucishu'){
            this.ruleForm.fuwucishu = obj[o];
	    this.ro.fuwucishu = true;
            continue;
          }
          if(o=='nianyejie'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jishi/jishigonghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jishigonghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    jishigonghaoChange () {
      this.$http({
        url: `follow/jishi/jishigonghao?columnValue=`+ this.ruleForm.jishigonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.jishixingming){
                this.ruleForm.jishixingming = data.data.jishixingming
              }
    ...
    info(id) {
      this.$http({
        url: `jishinianyejitongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jishinianyejitongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jishinianyejitongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
