<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="项目名称" prop="xiangmumingcheng">
          <el-select v-model="ruleForm.xiangmumingcheng" placeholder="请选择项目名称">
            <el-option
                v-for="(item,index) in xiangmumingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="预约详情" prop="yuyuexiangqing">
          <el-input v-model="ruleForm.yuyuexiangqing" 
              placeholder="预约详情" clearable  :readonly="ro.yuyuexiangqing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="预约详情" prop="yuyuexiangqing">
              <el-input v-model="ruleForm.yuyuexiangqing" 
                placeholder="预约详情" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="指定技师" prop="zhidingjishi">
          <el-select v-model="ruleForm.zhidingjishi" placeholder="请选择指定技师">
            <el-option
                v-for="(item,index) in zhidingjishiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="指定技师" prop="zhidingjishi">
	      <el-input v-model="ruleForm.zhidingjishi"
                placeholder="指定技师" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuyueshijian = true;
          ...
          if(o=='yuyuexiangqing'){
            this.ruleForm.yuyuexiangqing = obj[o];
	    this.ro.yuyuexiangqing = true;
            continue;
          }
          if(o=='zhidingjishi'){
            this.ruleForm.zhidingjishi = obj[o];
	    this.ro.zhidingjishi = true;
            continue;
          }
          if(o=='userid'){
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jishi/jishixingming`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhidingjishiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuanyuyuexinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuanyuyuexinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
