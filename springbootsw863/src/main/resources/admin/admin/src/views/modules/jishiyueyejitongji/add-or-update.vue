<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="月业绩额" prop="yueyejie">
          <el-input v-model="ruleForm.yueyejie" 
              placeholder="月业绩额" clearable  :readonly="ro.yueyejie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="月业绩额" prop="yueyejie">
              <el-input v-model="ruleForm.yueyejie" 
                placeholder="月业绩额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fuwucishu = true;
          ...
          if(o=='yueyejie'){
            this.ruleForm.yueyejie = obj[o];
	    this.ro.yueyejie = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `jishiyueyejitongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jishiyueyejitongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jishiyueyejitongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
