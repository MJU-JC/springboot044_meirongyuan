<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="会员账号" prop="huiyuanzhanghao">
          <el-select  @change="huiyuanzhanghaoChange" v-model="ruleForm.huiyuanzhanghao" placeholder="请选择会员账号">
            <el-option
                v-for="(item,index) in huiyuanzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.huiyuanzhanghao" label="会员账号" prop="huiyuanzhanghao">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="年消费额" prop="nianxiaofeie">
          <el-input v-model="ruleForm.nianxiaofeie" 
              placeholder="年消费额" clearable  :readonly="ro.nianxiaofeie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="年消费额" prop="nianxiaofeie">
              <el-input v-model="ruleForm.nianxiaofeie" 
                placeholder="年消费额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingbie = true;
          ...
          if(o=='nianxiaofeie'){
            this.ruleForm.nianxiaofeie = obj[o];
	    this.ro.nianxiaofeie = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/huiyuan/huiyuanzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.huiyuanzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    huiyuanzhanghaoChange () {
      this.$http({
        url: `follow/huiyuan/huiyuanzhanghao?columnValue=`+ this.ruleForm.huiyuanzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.huiyuanxingming){
                this.ruleForm.huiyuanxingming = data.data.huiyuanxingming
              }
    ...
    info(id) {
      this.$http({
        url: `huiyuannianxiaofeixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuannianxiaofeixinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuannianxiaofeixinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
