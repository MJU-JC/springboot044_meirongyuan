      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="前台账号" prop="qiantaizhanghao">
          <el-input v-model="ruleForm.qiantaizhanghao" 
              placeholder="前台账号" clearable  :readonly="ro.qiantaizhanghao"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	money : false,
      ...
      rules: {
          qiantaizhanghao: [
                { required: true, message: '前台账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          qiantaixingming: [
                { required: true, message: '前台姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          nianling: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `qiantai/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.qiantaiCrossAddOrUpdateFlag = false;
                  this.parent.search();
