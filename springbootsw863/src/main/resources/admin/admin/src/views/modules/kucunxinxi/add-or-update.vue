<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="统计时间" prop="tongjishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tongjishijian" 
                type="datetime"
                placeholder="统计时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tongjishijian" label="统计时间" prop="tongjishijian">
              <el-input v-model="ruleForm.tongjishijian" 
                placeholder="统计时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="库存余量" prop="kucunyuliang">
          <el-input v-model="ruleForm.kucunyuliang" 
              placeholder="库存余量" clearable  :readonly="ro.kucunyuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="库存余量" prop="kucunyuliang">
              <el-input v-model="ruleForm.kucunyuliang" 
                placeholder="库存余量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.tongjishijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.meirongpinming = true;
          ...
          if(o=='tongjishijian'){
            this.ruleForm.tongjishijian = obj[o];
	    this.ro.tongjishijian = true;
            continue;
          }
          if(o=='kucunyuliang'){
            this.ruleForm.kucunyuliang = obj[o];
	    this.ro.kucunyuliang = true;
            continue;
          }
          if(o=='leixing'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kucunxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kucunxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
