<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="月销量" prop="yuexiaoliang">
          <el-input v-model="ruleForm.yuexiaoliang" 
              placeholder="月销量" clearable  :readonly="ro.yuexiaoliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="月销量" prop="yuexiaoliang">
              <el-input v-model="ruleForm.yuexiaoliang" 
                placeholder="月销量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="月销售额" prop="yuexiaoshoue">
            <el-input v-model="yuexiaoshoue"
                placeholder="月销售额" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.yuexiaoshoue" label="月销售额" prop="yuexiaoshoue">
              <el-input v-model="ruleForm.yuexiaoshoue" 
                placeholder="月销售额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
...
export default {
  ...
  computed: {
    yuexiaoshoue:{
      get: function () {
        return 1*this.ruleForm.xiangmujiage*this.ruleForm.yuexiaoliang
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xiangmujiage = true;
          ...
          if(o=='yuexiaoliang'){
            this.ruleForm.yuexiaoliang = obj[o];
	    this.ro.yuexiaoliang = true;
            continue;
          }
          if(o=='yuexiaoshoue'){
            this.ruleForm.yuexiaoshoue = obj[o];
	    this.ro.yuexiaoshoue = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `meirongxiangmuyuetongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      // ${column.compare}
      // ${column.compare}
        this.ruleForm.yuexiaoshoue = this.yuexiaoshoue
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `meirongxiangmuyuetongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.meirongxiangmuyuetongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
